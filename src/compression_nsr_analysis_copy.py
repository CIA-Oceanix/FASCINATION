...
def compute_nsr_along_depth(
    ...
    Handles non-uniform depth sampling by interpolating to uniform grid using
    memory-efficient chunked operations (avoids creating full 4D intermediate copies).
...
def compute_nsr_spatial_map(
    ...
    Data is averaged over the depth (z) and time dimensions using efficient chunked operations.
    ...
    # Average over time and depth ITERATIVELY to avoid loading full arrays
    # Shape: (time, z, lat, lon) -> compute mean along time and z without creating full intermediate arrays
    truth_mean = truth_da.mean(dim=['time', 'z'], skipna=True).values  # (lat, lon)
    recon_mean = reconstructed_da.mean(dim=['time', 'z'], skipna=True).values  # (lat, lon)
    ...
    truth_flat = truth_mean.ravel()
    recon_flat = recon_mean.ravel()
    ...
    # Free temporary arrays
    del truth_valid, recon_valid, error, truth_flat, recon_flat, truth_mean, recon_mean
