...
def load_model_from_checkpoint(checkpoint_path: Path, device: str = "cpu", dm=None, batch=None):
    ...
    if "MLIC" in checkpoint_path_str:
        # MLIC format
        cfg = parse_mlic_config(checkpoint_path)
        ...
        if model.in_channels == 3:
            ...
            model_type = "MLIC"
    elif "AE" in checkpoint_path_str:
...
def make_checkpoint_tag(checkpoint_path: Path) -> str:
    checkpoint_path = Path(checkpoint_path)
    run_name = checkpoint_path.parents[1].name if len(checkpoint_path.parents) > 1 else checkpoint_path.parent.name
    checkpoint_name = checkpoint_path.name.replace(".pth.tar", "").replace(".tar", "")
    return f"{run_name}_{checkpoint_name}"
...
def gradient_variance_score(x_train):
    """
    Mean spatial gradient magnitude.
    ...
    if grad.shape[0] > 1:
        score = grad.var(dim=1, unbiased=False).mean(dim=0)
    else:
        score = grad.var(dim=1).squeeze()
...
def select_points_with_suppression_mask(
    score,
    n_points=500,
    radius=5,
):
    ...
    while len(selected) < n_points:
        ...
        dist2 = (yy - h)**2 + (xx - w)**2
        score[dist2 <= radius**2] = -float("inf")
...
def compute_sensor_score(
    ...
    for start in range(0, H*W, spatial_block):
...
def plot_optimization_history(reconstruction_dict,save_path=None):
    ...
    for results_per_day_dict in reconstruction_dict.values():
        ...
        sensor_mode = config.get("sensor_mode", "unknown")
        radius = config.get("radius", "unknown")
        loss_on = config["loss_on"]
        k = config["k"]

        label =  f"n_obs={n_obs}, sensor_mode={sensor_mode}, radius={radius},  lambda={lam}, k={k}"  #f"n_obs={n_obs}, lambda={lam}, y_hat_init={y_hat_init}, loss_on={loss_on}, k={k}"
        
        axs[0][0].plot(result['history']['loss'], label=label) 
        axs[1][0].plot(result['history']['obs_diff'], label=label)
        axs[0][1].plot(result['history']['latent_consistency'], label=label)
        axs[1][1].plot(result['history']['reconstruction_diff'], label=label)
...
def plot_profiles(reconstruction_dict, ssp_ae_tensor, truth_tensor, mask, depth_array, crop_idx=None, norm_stats="test", save_path=None):

    fig,ax = plt.subplots(1,1, figsize=(8,6))

    if crop_idx is not None:
        mask = mask[..., crop_idx, crop_idx]
        ssp_ae_tensor = ssp_ae_tensor[..., crop_idx, crop_idx]
        truth_tensor = truth_tensor[..., crop_idx, crop_idx]


    ssp_ae_np = unorm_ssp_arr_3D(ssp_ae_tensor.detach().cpu().numpy(), state[norm_stats + "_norm_stats"])
    truth_np = unorm_ssp_arr_3D(truth_tensor.detach().cpu().numpy(), state["test_norm_stats"])
    ...
    for results_per_day_dict in reconstruction_dict.values():
        ...
        label = f"n_obs={n_obs}, sensor_mode={sensor_mode}, radius={radius},  lambda={lam}, k={k}"  #y_hat_init={y_hat_init}, loss_on={loss_on},

        if crop_idx is not None:
            ssp = result['best_xhat'][:,:,crop_idx, crop_idx].detach().cpu().numpy()
        else:
            ssp = result['best_xhat'].detach().cpu().numpy()

        ssp = unorm_ssp_arr_3D(ssp, state[norm_stats + "_norm_stats"])

        ax.plot(ssp[t_idx,:,lat,lon], depth_array, label=label)


    ax.plot(truth_np[t_idx,:,lat,lon], depth_array, label=f"obs")
    ax.plot(ssp_ae_np[t_idx,:,lat,lon], depth_array, label=f"baseline reconstruction")
...
if __name__ == "__main__":
    ...
    checkpoint_paths = [
        "/Odyssey/private/o23gauvr/code/MLIC/experiments/test_mean_std_along_depth/",
    ]
    checkpoints = find_checkpoints(checkpoint_paths)
    if not checkpoints:
        raise ValueError(f"No checkpoints found in: {checkpoint_paths}")
    ...
    infer_batch_size = int(os.environ.get("MLIC_INFER_BATCH_SIZE", "32"))
    ...
    del ssp_truth_da, ssp_truth_da_unorm #, ssp_ae_da
    ...
    sensor_mode_list = ["grad_variance", "random"] #"grad_variance", "random","entropy", "latent_space_qr", "error_map", "variance", ,  #"latent_space_qr", "error_map", "variance"  "gradient", "grad_variance", "entropy", "baseline"       #variance #grad_variance # entropy #gradient #latent_space_qr, learned_soft, baseline
    radius_list = [10] #2,5,10,20,50

    date = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_root = Path(
        f"/Odyssey/private/o23gauvr/code/FASCINATION/experiments/latent_space_optimisation/results_test_full_loss_gauss/{date}"
    )
    results_root.mkdir(parents=True, exist_ok=True)

    results_dir = Path("/Odyssey/private/o23gauvr/code/FASCINATION")
    (results_dir / "pickle").mkdir(parents=True, exist_ok=True)

    for checkpoint_path in tqdm(checkpoints, desc="Processing checkpoints"):
        checkpoint_path = Path(checkpoint_path)
        checkpoint_tag = make_checkpoint_tag(checkpoint_path)
        model_device = device

        print(f"\nProcessing checkpoint: {checkpoint_path}")

        model, state, model_type, cfg = load_model_from_checkpoint(
            checkpoint_path,
            device=load_device,
            dm=dm,
            batch=None,
        ...
        ckpt_name = "_".join([w for w in ckpt_name.split("_") if w not in ["best", "checkpoint"]])
        lst = str(checkpoint_path).split('/')
        pattern = r"\d{4}-\d{2}-\d{2}_\d{2}-\d{2}|\d{8}_\d{6}"
        date_str = next((x for x in lst if re.fullmatch(pattern, x)), None).split("_")[-1]
        ...
        baseline_metrics = evaluate_full_span_reconstruction(
            ...
            crop_idx=crop_idx,
            mlic_path=checkpoint_path,
        )

        for sensor_mode in sensor_mode_list:

            if sensor_mode == "latent_space_qr":
                score = compute_sensor_score(
                    model,
                    x_train[::4],
                    crop_idx=crop_idx,
                    batch_size=infer_batch_size,
                ...
                #score = compute_sensor_score_fast(model, x_train[::4], crop_idx=slice(20,-20),n_probe=100, batch_size=infer_batch_size, device=model_device)

            elif sensor_mode in ["error_map", "variance", "gradient", "grad_variance", "entropy"]:
                score = torch.zeros(H, W, device="cpu", dtype=torch.float32)

            for radius in radius_list:
                for y_hat_init in y_hat_inits:
                    k_values = nn_k_values if y_hat_init == "nn" else [None]
                    for k in k_values: #, desc=f"y_hat_init={y_hat_init}"):
                        for loss_on in loss_on_values:
                            for lam in lam_values:
                                for n_obs_per_day in n_obs_per_day_values:

                                    config_key = (
                                        f"model: {model_name}|sensor_mode={sensor_mode}|"
                                       
                                    ) # nobs={n_obs_per_day}|radius={radius}|init={y_hat_init}|"f"k={k if k is not None else 'na'}|loss={loss_on}|lam={lam}"

                                    results.setdefault(config_key, {})

                                    for t_idx in days_idx_top_optimise:

                                        obs_sample = truth_tensor[t_idx : t_idx + 1].to(model_device)  # shape [1, C, H, W]

                                        unorm_obs_sample = unorm_ssp_arr_3D(
                                            obs_sample.detach().cpu().numpy(),
                                            state["test_norm_stats"],
                                        )
                                        unorm_rec_sample = unorm_ssp_arr_3D(
                                            ssp_ae_tensor[t_idx : t_idx + 1].detach().cpu().numpy(),
                                            state[norm_stats + "_norm_stats"],
                                        )

                                        if crop_idx is not None:
                                            unorm_obs_sample = unorm_obs_sample[..., crop_idx, crop_idx]
                                            unorm_rec_sample = unorm_rec_sample[..., crop_idx, crop_idx]

                                        unorm_obs_sample_t = torch.as_tensor(
                                            unorm_obs_sample,
                                            device=score.device,
                                            dtype=score.dtype,
                                        )
                                        unorm_rec_sample_t = torch.as_tensor(
                                            unorm_rec_sample,
                                            device=score.device,
                                            dtype=score.dtype,
                                        )
                                        ...
                                        if sensor_mode == "latent_space_qr":
                                        ...
                                        elif sensor_mode == "error_map":
                                            error_map = torch.abs(unorm_rec_sample_t - unorm_obs_sample_t).mean(dim=1, keepdim=False).squeeze()
                                            score[crop_idx, crop_idx] = error_map
                                            mask, _ = select_points_with_suppression_mask(
                                                score,
                                                n_obs_per_day,
                                                radius=radius,
                                            )
                                        ...
                                        elif sensor_mode == "variance":
                                            train_variance = torch.var(unorm_obs_sample_t, dim=1, unbiased=False).squeeze()
                                            score[crop_idx, crop_idx] = train_variance
                                            mask, _ = select_points_with_suppression_mask(
                                                score,
                                                n_obs_per_day,
                                                radius=radius,
                                        ...
                                        elif sensor_mode == "gradient":
                                            grad_score = gradient_score(unorm_obs_sample_t)
                                            score[crop_idx, crop_idx] = grad_score
                                            mask, _ = select_points_with_suppression_mask(
                                                score,
                                                n_obs_per_day,
                                                radius=radius,
                                        ...
                                        elif sensor_mode == "grad_variance":
                                            grad_var = gradient_variance_score(unorm_obs_sample_t)
                                            score[crop_idx, crop_idx] = grad_var
                                            mask, _ = select_points_with_suppression_mask(
                                                score,
                                                n_obs_per_day,
                                                radius=radius,
                                        ...
                                        elif sensor_mode == "entropy":
                                            entr_score = entropy_score(unorm_obs_sample_t.transpose(1,0))
                                            score[crop_idx, crop_idx] = entr_score
                                            mask, _ = select_points_with_suppression_mask(
                                                score,
                                                n_obs_per_day,
                                                radius=radius,
                                            ...
                                            mask = torch.zeros_like(obs_sample, dtype=torch.float32)
                                            obs_points = make_spatial_observation_mask(
                                                obs_sample[:, :, crop_idx, crop_idx],
                                                n_obs=n_obs_per_day,
                                                seed=42,
                                            )
                                            mask[:, :, crop_idx, crop_idx] = obs_points
        ...
        selected_days_rmse_df = selected_days_summary_to_dataframe(
            selected_days_rmse_summary,
            baseline_metrics,
        )

        pickle_path = results_dir / "pickle" / f"z_optimization_cadence_results_{checkpoint_tag}.pkl"
        ...
        for config_key, summary in selected_days_rmse_summary.items():
            print(
                f"[{checkpoint_tag}] {config_key} | global_rec_rmse={summary['global_rec_rmse']:.6e} | "
                f"rec_rmse_std={summary['rec_rmse_distribution']['std']:.6e}"
            )

            if summary['global_obs_rmse'] is None or summary['global_rec_rmse'] is None:
                print(f"[{checkpoint_tag}] {config_key} | global_obs_rmse=None | obs_rmse_std=None | global_rec_rmse=None | rec_rmse_std=None")
                continue
            print(
                f"[{checkpoint_tag}] {config_key} | global_obs_rmse={summary['global_obs_rmse']:.6e} | "
                f"obs_rmse_std={summary['obs_rmse_distribution']['std']:.6e}"
            )

        summary_results_dir = results_root / checkpoint_tag
