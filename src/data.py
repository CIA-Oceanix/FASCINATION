...
class AutoEncoderDatamodule(pl.LightningDataModule):
    def __init__(self, input_da, domains, dl_kw, x_min = 1438, x_max = 1552.54994512, pickle_path = None):
        super().__init__()
        ...
        if self.pickle_path:
            ...
            with open(self.pickle_path,"wb") as f:
                pickle.dump(
                    dict(
                        train=self.input_da.isel(self.domains['train']).coords['time'].values,
                        val=self.input_da.isel(self.domains['val']).coords['time'].values,
                        test=self.input_da.isel(self.domains['test']).coords['time'].values
                    ),
                    f
                )
